...
La función [aggregate()](https://rdrr.io/r/stats/aggregate.html) del paquete ```stats``` aplica una función de agregación (ej. suma, promedio, mínimo, máximo) sobre una columna. El resultado es un objeto de tipo ```data.frame```.

```{r}
# Suma de áreas de cantones por provincia
aggregate(data = cr_cantones, area ~ provincia, FUN = sum, na.rm = TRUE)
```
...
```{r}
# Suma de áreas de cantones por provincia
aggregate(cr_cantones["area"], by = list(cr_cantones$provincia), FUN = sum, na.rm = TRUE)
```
